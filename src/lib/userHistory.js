...
const BUG_REPORTS_KEY = 'tcw_my_bug_reports';
...
export function saveMyBugReport(id) {
  const list = getMyBugReportIds();
  if (!list.includes(id)) {
    list.unshift(id);
    localStorage.setItem(BUG_REPORTS_KEY, JSON.stringify(list));
  }
}
...
export function getMyBugReportIds() {
  try { return JSON.parse(localStorage.getItem(BUG_REPORTS_KEY)) || []; }
  catch { return []; }
}
...
export function removeMyBugReport(id) {
  const list = getMyBugReportIds().filter(b => b !== id);
  localStorage.setItem(BUG_REPORTS_KEY, JSON.stringify(list));
}