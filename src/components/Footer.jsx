import { Facebook, Github, Instagram, Mail, Music2, LogIn, LogOut, ShieldCheck } from "lucide-react";
      ...
      <div className="max-w-6xl mx-auto border-t border-[#2d2d2d] px-8 py-6">
        <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
          <a href="https://vnu.edu.vn" target="_blank" rel="noopener noreferrer" className="inline-block">
            <div className="font-lexend font-black text-2xl leading-none tracking-wide text-[#16a34a]">VNU</div>
            <div className="mt-1 text-[10px] font-bold uppercase leading-none text-[#16a34a]">&#272;&#7840;I H&#7884;C QU&#7888;C GIA H&#192; N&#7896;I</div>
            <div className="mt-1 text-[10px] font-medium leading-none text-gray-300">Vietnam National University, Hanoi</div>
          </a>
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            <a href={emailLink.href} title={emailLink.label}
              className="w-11 h-11 rounded-full bg-[#242424] border border-[#3a3a3a] flex items-center justify-center text-gray-200 hover:bg-[#303030] hover:border-[#555] transition-all hover:-translate-y-0.5">
              {emailLink.icon}
            </a>
            <div className="h-8 w-px bg-[#3a3a3a] mx-2" />
              ...
              <div className="flex items-center gap-2">
                {user?.email === ADMIN_EMAIL && (
                  <Link to="/admin/tai-lieu"
                    className="flex items-center gap-1.5 px-3 py-2 bg-yellow-400 text-black rounded-xl font-bold text-xs hover:bg-yellow-300 transition-all">
                    <ShieldCheck className="w-4 h-4" /> Admin
                  </Link>
                )}
                <button onClick={() => logout()}
                  className="flex items-center gap-1.5 px-3 py-2 bg-[#242424] border border-[#3a3a3a] text-gray-200 rounded-xl font-bold text-xs hover:bg-[#303030] transition-all">
